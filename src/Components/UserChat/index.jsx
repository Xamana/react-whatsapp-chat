...
export const UserChat = (props) => {
  const { inputMessageValue, setInputMessageValue, sendMessage, currentContact } = props;
  const messages = useSelector((state) => state.chatsSlice.chats);
    ...
    <div className={s.chat}>
      <div className={s.messages}>
        <div className={s.userInfo}>
          ...
          <div className={s.userName}>{currentContact}</div>
        </div>
        <div className={s.chatWindow}>
          {messages[currentContact].messages ? ( 
            messages[currentContact].messages.map((obj, i) => (
              <div key={i} ref={messagesRef} className={`${s.messageBlock} ${obj.sender ? s.me : s.from}` }>
                {obj.message}
              </div>
            ))
          ) : (
            <div>Упс с данныйм пользователем у вас нет сообщений...</div>
          )}
        </div>
        <div className={s.messageForm}>
          ...
          />
          <button className={s.inputButton} onClick={sendMessageClick}>
